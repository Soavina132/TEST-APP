...
BEGIN
  ...
  -- Respect thinking delay. If not armed for THIS slot, arm it and return.
  ...
  IF v_think_until IS NULL OR v_locked_slot IS DISTINCT FROM v_slot THEN
    -- Random 1.5-3.5s "reflection" window
  ...
  -- Delay elapsed: clear lock and play exactly ONE move for this bot.
  ...
  IF found THEN
    ...
    IF is_first_move THEN
      st := jsonb_set(st, '{board}', jsonb_build_array(jsonb_build_object('tile', tile, 'flipped', false)), true);
      new_left := a;
      new_right := b;
    ELSE
      IF a = re OR b = re THEN
        ...
        new_left := le;
        st := jsonb_set(st, '{board}', COALESCE(st->'board','[]'::jsonb) || jsonb_build_array(jsonb_build_object('tile', tile, 'flipped', a = re AND a <> b)), true);
      ELSE
        IF a = le THEN new_left := b; ELSE new_left := a; END IF;
        new_right := re;
        st := jsonb_set(st, '{board}', jsonb_build_array(jsonb_build_object('tile', tile, 'flipped', a <> le)) || COALESCE(st->'board','[]'::jsonb), true);
      END IF;
    END IF;
    ...
    -- Arm think delay for the next slot if it is also a bot
  ...
  IF draw_mode = 'with' AND jsonb_array_length(stock) > 0 THEN
    ...
    -- Re-arm short delay to "draw again / play" so the next tick picks it up
