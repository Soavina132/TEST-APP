-- ═══════════════════════════════════════════════════════════════════════
-- FIX v8 — domino_play déclenche le bot automatiquement
--
-- Problème: le frontend Lovable appelle domino_play (pas domino_play_and_bot).
-- domino_play arme bot_think_until mais ne fait jamais jouer le bot.
-- Le bot reste bloqué jusqu'au cron (1 min) ou au frontend realtime.
--
-- Solution: domino_play fait maintenant la boucle bot elle-même.
-- Après chaque action humaine (play/draw/pass), si le prochain tour est
-- un bot, on attend le think delay (pg_sleep) puis on appelle domino_tick.
...
BEGIN
  ...
  IF action = 'draw' THEN
    ...
    UPDATE public.domino_games
     ...
     WHERE id = _game_id;
    -- Bot loop: si le tirage donne une tuile non jouable, c'est toujours
    -- le tour du joueur, donc pas de bot à déclencher. On retourne.
    RETURN;
  END IF;
  ...
  -- → Bot loop: faire jouer le bot immédiatement
  PERFORM public._domino_bot_loop(_game_id);
END $function$;

-- ═════ Helper: boucle bot (arm → sleep → tick → repeat) ═══════════════
