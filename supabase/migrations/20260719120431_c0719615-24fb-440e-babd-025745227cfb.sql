...
BEGIN
  ...
  VALUES
    (v_admin,'ftest_admin_'||substr(v_admin::text,1,8),'Admin Test',0),
    (v_p1,   'ftest_p1_'   ||substr(v_p1::text,1,8),   'Joueur 1',  0),
    (v_p2,   'ftest_p2_'   ||substr(v_p2::text,1,8),   'Joueur 2',  0);
  ...
  INSERT INTO public.deposits(user_id,amount,method,reference,user_phone,status)
    VALUES (v_p1,10000,'mvola','FT-1','0340000000','pending')
  ...
  IF v_bal <> 10000 THEN RAISE EXCEPTION 'FAIL T1: attendu 10000, obtenu %', v_bal; END IF;
  v_report := v_report || E'PASS T1 — Dépôt approuvé crédite le solde\n';

  -- T2 : Dépôt + dette antérieure
  INSERT INTO public.withdrawal_debts(user_id, amount_ar) VALUES (v_p2, 3000);
  INSERT INTO public.deposits(user_id,amount,method,reference,user_phone,status)
    VALUES (v_p2,5000,'mvola','FT-2','0340000001','pending')
    RETURNING id INTO v_dep_id;
  PERFORM public.admin_process_deposit(v_dep_id, true);
  ...
  IF v_bal <> 2000 OR v_debt <> 0 THEN
    RAISE EXCEPTION 'FAIL T2: solde=% (attendu 2000) dette=% (attendu 0)', v_bal, v_debt;
  END IF;
  v_report := v_report || E'PASS T2 — Dépôt éponge la dette avant de créditer\n';

  -- T3 : Retrait approuvé → solde = 0 + balance_reset
  ...
  END;
  PERFORM set_config('request.jwt.claim.sub', v_admin::text, true);
  IF NOT v_ok THEN RAISE EXCEPTION 'FAIL T4: request_withdrawal aurait dû échouer'; END IF;
  ...
  IF v_bal <> v_net THEN
    RAISE EXCEPTION 'FAIL T5 Ludo: attendu %, obtenu % (commission % pct)', v_net, v_bal, v_cpct;
  END IF;
  ...
  IF v_bal <> v_net THEN RAISE EXCEPTION 'FAIL T6 Chess win: attendu %, obtenu %', v_net, v_bal; END IF;
  ...
  -- Cleanup : cascade via auth.users
...
EXCEPTION WHEN OTHERS THEN
  -- Best-effort cleanup avant de repropager
  BEGIN DELETE FROM auth.users WHERE id IN (v_admin, v_p1, v_p2); EXCEPTION WHEN OTHERS THEN NULL; END;
  RAISE;
END
$fn$;

REVOKE ALL ON FUNCTION public.run_finance_tests() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.run_finance_tests() TO authenticated, service_role;
