-- Fix: domino_play appended every played tile to the END of state.board,
-- even when the tile was placed on the LEFT side of the chain. The client
-- renders board[] left-to-right assuming visual chain order, so a left-play
-- ended up drawn at the right end with a broken connection (e.g. a [5|5]
-- double visually adjacent to a [2|5] tile with the 2 touching the 5).
--
-- Fix: prepend to the array when side='left', append when side='right',
-- so state.board is always in visual chain order (leftEnd → rightEnd).
...
BEGIN
  ...
  IF le IS NULL THEN
    ...
    -- First tile: initialise board with just this tile.
    st := jsonb_set(st, '{board}', jsonb_build_array(jsonb_build_object('tile', tile, 'flipped', false)));
  ELSE
    -- Auto-select side when caller omitted it. IMPORTANT: assign `side`
    -- explicitly on match so the final CASE keeps the newly computed end.
    ...
    -- Keep state.board in VISUAL chain order (leftEnd → rightEnd).
    -- Prepend to array head when playing on the left side, append on the right.
    placed_tile := jsonb_build_object('tile', tile, 'flipped', false);
    IF side = 'left' THEN
      st := jsonb_set(st, '{board}', jsonb_build_array(placed_tile) || (st->'board'));
    ELSE
      st := jsonb_set(st, '{board}', (st->'board') || jsonb_build_array(placed_tile));
    END IF;
  END IF;
