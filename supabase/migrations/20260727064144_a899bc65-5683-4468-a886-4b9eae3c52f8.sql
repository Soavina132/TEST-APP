...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════════════════════
  -- Petite finale automatique : si le prochain round est LA FINALE
  -- (exactement 2 qualifiés issus de demi-finales 1v1), créer aussi
  -- un match "is_third_place" entre les 2 perdants de demi-finale.
  -- ═══════════════════════════════════════════════════════════════════════
  IF v_count = 2 AND COALESCE(trn.players_per_match, 2) = 2 THEN
    v_losers := '{}';
    FOR lm IN
      SELECT player_ids, winner_id, array_length(player_ids,1) AS n
        FROM public.tournament_matches
       WHERE tournament_id = _tid
         AND round = trn.current_round
         ...
         AND COALESCE(is_third_place,false) = false
    LOOP
      IF COALESCE(lm.n,2) = 2 AND lm.winner_id IS NOT NULL THEN
        FOREACH lp IN ARRAY lm.player_ids LOOP
          IF lp <> lm.winner_id THEN
    ...
    IF array_length(v_losers,1) = 2
      ...
      INSERT INTO public.tournament_matches(
        ...
        _tid, v_next_round, v_losers, 'pending', false, 1, true
      );
    END IF;
  END IF;
