...
BEGIN
  ...
  ELSIF _action = 'draw' THEN
    ...
    ELSIF jsonb_array_length(_stock) > 0 THEN
      -- Not playable but stock remains: keep turn and draw again after short delay
      UPDATE public.domino_games SET state = _state, turn_deadline = now() + make_interval(secs => 2), updated_at = now() WHERE id = _game_id;
      -- Bot continues drawing from backend without waiting for frontend tick
      PERFORM pg_sleep(0.5);
      PERFORM public.domino_bot_play(_game_id, _bot);
    ELSE
      -- Stock empty: pass
