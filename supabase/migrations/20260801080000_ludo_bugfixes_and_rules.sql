-- ============================================================
-- Migration: Correctifs Ludo — bugs critiques + règles
-- Date: 2026-08-01
-- Description: 
--   1. Guard contre timestamps corrompus (jeu bloqué 2126)
--   2. Re-check status après forfeit dans ludo_check_timeout
--   3. _ludo_check_last_standing termine les parties bot-only
--   4. Champs turn_seq/phase/spin_ms dans toutes les fonctions
--   5. ludo_bot_play re-vérifie le status après move
--   6. Règle des 3 x 6 consécutifs = skip du tour
-- ============================================================

-- 1. ludo_check_timeout — guard timestamp + re-check status
CREATE OR REPLACE FUNCTION public.ludo_check_timeout(_game_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public'
AS $function$
DECLARE g public.ludo_games%ROWTYPE; st jsonb; v_slot INT; v_started TIMESTAMPTZ; v_uid UUID; v_isbot BOOLEAN; v_missed INT; v_winner UUID; v_status TEXT;
BEGIN
  SELECT * INTO g FROM public.ludo_games WHERE id=_game_id FOR UPDATE;
  IF g.status <> 'playing' THEN RETURN g.state; END IF;
  st := g.state;
  v_started := (st->>'turn_started_at')::TIMESTAMPTZ;
  IF v_started > now() + interval '1 minute' THEN v_started := now() - interval '60 seconds'; END IF;
  IF now() - v_started < interval '30 seconds' THEN RETURN st; END IF;
  v_slot := (st->>'turn_slot')::INT;
  SELECT user_id, is_bot, missed_turns INTO v_uid, v_isbot, v_missed FROM public.ludo_participants WHERE game_id=_game_id AND slot=v_slot;
  ...
  IF v_missed >= 3 AND NOT v_isbot THEN
    ...
    INSERT INTO public.transactions(user_id,type,amount,ref_id,note) VALUES (v_uid,'forfeit',0,_game_id,'Forfait (3 timeouts)');
    SELECT status INTO v_status FROM public.ludo_games WHERE id=_game_id;
    IF v_status = 'finished' THEN RETURN (SELECT state FROM public.ludo_games WHERE id=_game_id); END IF;
    v_winner := public._ludo_check_last_standing(_game_id);
    IF v_winner IS NOT NULL THEN PERFORM public.finish_game(_game_id, v_winner); RETURN (SELECT state FROM public.ludo_games WHERE id=_game_id); END IF;
  END IF;
...
-- 2. _ludo_check_last_standing — gère le cas bot-only
...
AS $function$
DECLARE v_count INT; v_uid UUID; v_has_humans BOOLEAN;
BEGIN
  SELECT count(*) INTO v_count FROM public.ludo_participants WHERE game_id=_game_id AND forfeited=FALSE;
  IF v_count <= 1 THEN
    SELECT user_id INTO v_uid FROM public.ludo_participants WHERE game_id=_game_id AND forfeited=FALSE AND is_bot=FALSE LIMIT 1;
    IF v_uid IS NOT NULL THEN RETURN v_uid; END IF;
    SELECT EXISTS(SELECT 1 FROM public.ludo_participants WHERE game_id=_game_id AND is_bot=FALSE AND forfeited=FALSE) INTO v_has_humans;
    IF NOT v_has_humans THEN
      UPDATE public.ludo_games SET status='finished', finished_at=now() WHERE id=_game_id AND status='playing';
      RETURN NULL;
    END IF;
  END IF;
  RETURN NULL;
...
-- 3. ludo_move — 3x6 consécutifs + champs turn_seq/phase/spin_ms
...
BEGIN
  ...
  v_seq := COALESCE((st->>'turn_seq')::int, 0) + 1;
  IF bonus THEN
    st := jsonb_set(st,'{last_event}', to_jsonb(CASE WHEN captured THEN 'capture'::text WHEN finished THEN 'home'::text ELSE 'six'::text END || ':rejoue'));
    st := jsonb_set(st,'{phase}', to_jsonb('spinning'::text));
...
-- 4. ludo_roll — champs phase/phase_started_at
CREATE OR REPLACE FUNCTION public.ludo_roll(_game_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public'
AS $function$
DECLARE st jsonb; g public.ludo_games%ROWTYPE; v_uid UUID := auth.uid();
  v_slot INT; v_user UUID; v_isbot BOOLEAN; v_bias INT; v_dice INT; v_now TEXT; v_seq INT;
BEGIN
  SELECT * INTO g FROM public.ludo_games WHERE id=_game_id FOR UPDATE;
  IF g.status <> 'playing' THEN RAISE EXCEPTION 'Partie pas en cours'; END IF;
  st := public._ludo_ensure_state(_game_id);
  v_slot := (st->>'turn_slot')::INT;
  SELECT user_id, is_bot, bot_win_bias INTO v_user, v_isbot, v_bias FROM public.ludo_participants WHERE game_id=_game_id AND slot=v_slot;
  IF NOT v_isbot AND v_user <> v_uid THEN RAISE EXCEPTION 'Pas votre tour'; END IF;
  IF (st->>'must_move')::BOOLEAN THEN RAISE EXCEPTION 'Deja lance, deplacez un pion'; END IF;
  v_dice := 1 + (floor(random()*6))::INT;
  IF v_isbot AND COALESCE(v_bias,0) > 0 AND (random()*100) < v_bias AND v_dice < 4 THEN v_dice := 3 + (floor(random()*4))::INT; END IF;
  v_now := to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"');
...
-- 5. ludo_pass — compteur 6 + champs turn_seq/phase/spin_ms
CREATE OR REPLACE FUNCTION public.ludo_pass(_game_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = 'public'
AS $function$
DECLARE g public.ludo_games%ROWTYPE; st jsonb; v_slot INT; v_dice INT; v_uid UUID := auth.uid(); v_user UUID; v_isbot BOOLEAN;
...
-- 6. ludo_bot_play — 3x6 + champs + re-check status
...
DECLARE
  ...
  other_slot INT; op jsonb; op_step INT; op_start INT;
