...
BEGIN
  ...
  IF g.host_id <> v_uid
     AND NOT EXISTS (SELECT 1 FROM public.domino_participants WHERE game_id=_game_id AND user_id=v_uid)
  ...
  INSERT INTO public.domino_participants(
    ...
    _game_id, NULL, v_slot, v_name, TRUE, TRUE, v_name, 70
  );

  -- Auto-start if now full and everyone is ready
...
REVOKE EXECUTE ON FUNCTION public.domino_add_bot(uuid, text) FROM anon, PUBLIC;
