...
BEGIN
  ...
  INSERT INTO public.profiles(id, pseudo, email, referral_code, balance_ar) VALUES
    (v_admin,'ftest_admin_'||substr(v_admin::text,1,8),'ftest-admin+'||v_admin::text||'@lovable.local','FTA'||substr(replace(v_admin::text,'-',''),1,8),0),
    (v_p1,   'ftest_p1_'   ||substr(v_p1::text,1,8),   'ftest-p1+'   ||v_p1::text   ||'@lovable.local','FT1'||substr(replace(v_p1::text,'-',''),1,8),0),
    (v_p2,   'ftest_p2_'   ||substr(v_p2::text,1,8),   'ftest-p2+'   ||v_p2::text   ||'@lovable.local','FT2'||substr(replace(v_p2::text,'-',''),1,8),0);
