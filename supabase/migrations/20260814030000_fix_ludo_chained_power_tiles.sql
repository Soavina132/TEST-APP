-- ============================================================
-- FIX: Chained power tiles — when a boost lands the pawn on
-- another power tile, that tile must ALSO be applied.
-- Previously _ludo_check_power_tile only ran once, so a boost
-- that moved the pawn onto another tile silently ignored it.
--
-- Also: ludo_move only re-checked capture when the power_event
-- type was 'boost'. If a chain was boost→shield, the last event
-- was 'shield' and the capture re-check was skipped. Now we
-- always re-check capture at the actual final pawn position.
-- ============================================================

-- 1) _ludo_check_power_tile: loop to chain power tiles after boost
