...
-- Date: 2026-08-14
-- ═══════════════════════════════════════════════════════════════════
-- Aligns domino_set_ready with ludo_set_ready:
-- If stake > 0 AND game is NOT solo (has human opponents),
-- require phone_verified = true before marking ready.
-- Solo games with bots are exempt (matching ludo behavior).
-- ═══════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  _v_uid uuid := auth.uid();
  _v_verified boolean;
  _v_human_count int;
BEGIN
  IF _v_uid IS NULL THEN RAISE EXCEPTION 'Non authentifié'; END IF;

  SELECT * INTO _g FROM public.domino_games WHERE id = _game_id;
  IF NOT FOUND OR _g.status != 'open' THEN RETURN; END IF;

  -- Phone verification check: required when stake > 0 AND there are other human players
  IF _ready AND COALESCE(_g.stake, 0) > 0 THEN
    SELECT count(*) INTO _v_human_count
    FROM public.domino_participants
    WHERE game_id = _game_id AND forfeited = false AND is_bot = false AND user_id != _v_uid;
    IF _v_human_count > 0 THEN
      SELECT phone_verified INTO _v_verified FROM public.profiles WHERE id = _v_uid;
  ...
  UPDATE public.domino_participants SET ready = _ready
    WHERE game_id = _game_id AND user_id = _v_uid AND forfeited = false;

  SELECT bool_and(ready), count(*) INTO _all, _count
...
REVOKE EXECUTE ON FUNCTION public.domino_set_ready(uuid, boolean) FROM PUBLIC;
