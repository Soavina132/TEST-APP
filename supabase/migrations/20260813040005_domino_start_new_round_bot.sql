...
DECLARE
  ...
  _delay interval; _starter_slot int; _starter_part record; _is_bot boolean := false;
BEGIN
  ...
  _starter_slot := (_info->>'slot')::int;

  UPDATE public.domino_games SET
    state = _state, current_turn = _starter_slot,
    turn_deadline = now() + _delay, updated_at = now()
  WHERE id = _game_id;

  -- If starter is a bot, trigger it immediately
  SELECT * INTO _starter_part FROM public.domino_participants WHERE game_id = _game_id AND slot = _starter_slot AND forfeited = false;
  _is_bot := COALESCE(_starter_part.is_bot, false);
  IF _is_bot THEN
    PERFORM pg_sleep(1 + random());
    PERFORM public.domino_bot_play(_game_id, _starter_part);
  END IF;
END;
$function$;
