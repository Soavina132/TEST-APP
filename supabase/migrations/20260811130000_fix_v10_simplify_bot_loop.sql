-- ═══════════════════════════════════════════════════════════════════════
-- FIX v10 — Simplification du bot loop
--
-- Problème: deux boucles imbriquées (domino_play → _domino_bot_loop
-- ET domino_play_and_bot → sa propre boucle) causent des conflits.
-- Le bot ne joue pas et attend 30 secondes (turn_deadline + cron).
--
-- Solution:
-- 1. Retirer _domino_bot_loop de domino_play (une seule boucle suffit)
-- 2. Réécrire domino_play_and_bot avec pg_sleep(1) entre les ticks
-- 3. La boucle fait: tick → sleep 1s → tick (le bot joue) → repeat
-- ═══════════════════════════════════════════════════════════════════════

-- ═══ 1. domino_play SANS _domino_bot_loop ═══
...
-- ═══ 2. domino_play_and_bot avec boucle simple et robuste ═══
...
BEGIN
  ...
  LOOP
    ...
    -- Tick (arme le bot si pas armé, ou fait jouer si think_until passé)
    PERFORM public.domino_tick(_game_id);

    -- Attendre 1 seconde (le think delay est 400-1000ms)
    PERFORM pg_sleep(1);
