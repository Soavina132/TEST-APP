...
let muted = false;
try {
  muted = localStorage.getItem("game_sound_muted") === "1";
} catch {
}
function getCtx() {
...
function noise(duration, volume = 0.3, filterFreq = 2e3, startAt = 0, filterType = "lowpass", useReverb = false) {
  if (muted) return;
  const c = getCtx();
  if (!c || !masterGain) return;
  const t0 = c.currentTime + startAt;
...
function chord(freqs, duration, type = "sine", volume = 0.3, startAt = 0, useReverb = false) {
  freqs.forEach((f) => tone({ freq: f, duration, type, volume, startAt, useReverb }));
}
function arpeggio(freqs, noteDuration, type = "sine", volume = 0.25, useReverb = false) {
  freqs.forEach((f, i) => {
...
const sfx = {
  ...
  pawnMove() {
    ...
    tone({ freq: 800, duration: 0.08, type: "triangle", volume: 0.1, startAt: 0.02, freqEnd: 1e3 });
  },
  ...
  capture() {
    ...
    noise(0.15, 0.2, 2e3, 0.05, "highpass");
  ...
  powerBoost() {
    ...
    arpeggio([800, 1e3, 1200, 1600], 0.08, "triangle", 0.12, true);
  },
  ...
  powerDoubleRoll() {
    ...
    tone({ freq: 1200, duration: 0.04, type: "square", volume: 0.22, startAt: 0.1 });
    ...
    noise(0.2, 0.08, 4e3, 0.18, "bandpass");
  },
  ...
  win() {
    ...
    chord([523, 659, 784, 1047], 1, "sine", 0.2, 0.5, true);
  ...
  lose() {
    ...
    tone({ freq: 110, duration: 1, type: "sine", volume: 0.1, startAt: 0.3, freqEnd: 55, useReverb: true });
  },
