...
import { Radio, Eye, EyeOff, Megaphone, Bell, Headphones } from "lucide-react";
...
function LobbyPage() {
  ...
  useEffect(() => {
    ...
    supabase.from("transactions")
      ...
      .limit(1)
      .then(({ data }) => {
        if (data && data.length > 0) setBonusAmount(Number((data as any[])[0].amount) || 0);
      });
  }, [user]);
  ...
  return (
    <main className="max-w-md mx-auto px-3 py-2 pb-24 space-y-2.5">
      ...
      {/* Bandeau défilant — événements/tournois */}
      <EventTicker />
      ...
      <div className="rounded-2xl bg-primary text-primary-foreground p-4 shadow-md shadow-primary/20">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setHideBalance(h => !h)}
                className="opacity-80 hover:opacity-100 active:scale-90 transition-all"
            ...
            <p className="text-3xl font-black tabular-nums leading-tight truncate">
              {hideBalance ? "•••• Ar" : fmtAr(balance)}
            </p>
            {/* Dernière mise à jour */}
            {balanceUpdatedAt && !hideBalance && (
              <p className="text-[9px] opacity-60 mt-0.5">
                Mis à jour il y a {Math.max(1, Math.round((Date.now() - balanceUpdatedAt) / 1000))} s
              </p>
            )}
            {/* Bonus disponible */}
            {bonusAmount > 0 && !hideBalance && (
              <p className="text-sm font-bold text-emerald-300 mt-1">
                +{fmtAr(bonusAmount)} Bonus 🎁
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          <button onClick={() => setShowDeposit(true)}
            className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white/20 font-bold text-sm active:scale-95 active:bg-white/30 transition-all">
          ...
          <button onClick={() => navigate({ to: "/history" as any })}
            className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white/20 font-bold text-sm active:scale-95 active:bg-white/30 transition-all">
            <History className="w-4 h-4 shrink-0" /> Historique
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <Link key={to} to={to}
            className="flex flex-col items-center gap-1 py-2 px-1 rounded-2xl bg-card border border-border active:scale-95 active:bg-secondary transition-all">
        ...
        {[
          { icon: Gift,     label: "Bonus",       to: "/parrainage" as const,  color: "text-orange-500" },
          { icon: Megaphone, label: "Annonces",  to: "/faq" as const,      color: "text-blue-500" },
        ].map(({ icon: Icon, label, to, color }) => (
          <Link key={`extra-${label}`} to={to}
            className="flex flex-col items-center gap-1 py-2 px-1 rounded-2xl bg-card border border-border active:scale-95 active:bg-secondary transition-all">
            <div className={`w-8 h-8 rounded-xl bg-secondary grid place-items-center ${color}`}>
              <Icon className="w-4 h-4" />
            </div>
      ...
      <section>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 mb-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button onClick={loadGames} aria-label="Rafraîchir"
              className="w-7 h-7 rounded-lg grid place-items-center active:bg-secondary text-muted-foreground">
              <RefreshCw className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setShowMesParties(true)}
              className="relative flex items-center gap-1 px-2.5 h-7 rounded-full bg-primary text-primary-foreground font-bold text-[11px]">
              📂 Mes parties
              {myOngoingCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-red-500 border border-background text-[9px] font-black grid place-items-center animate-pulse">
                  {myOngoingCount > 9 ? "9+" : myOngoingCount}
                </span>
              )}
            </button>
          </div>
        ...
        {liveAvailable > 0 && (
          <Link to="/live"
            className="mb-2 flex items-center gap-2 rounded-2xl p-2.5 bg-gradient-to-r from-red-500/15 via-red-500/10 to-transparent border border-red-500/25 active:scale-[0.98] transition-transform">
            <span className="w-8 h-8 rounded-xl bg-red-500/20 grid place-items-center shrink-0">
              <Radio className="w-4 h-4 text-red-500" />
            </span>
            <div className="min-w-0 flex-1">
              <div className="font-black text-xs flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
                LIVE — {liveAvailable} partie{liveAvailable > 1 ? "s" : ""} en direct
              </div>
              <div className="text-[10px] text-muted-foreground">Regarder maintenant</div>
            </div>
        ...
        {games.length === 0 ? (
          <div className="rounded-2xl bg-card border border-border p-5 text-center space-y-3">
            ...
            <div>
              <p className="font-bold text-sm text-foreground">Aucune partie en cours</p>
              <p className="text-[10px] text-muted-foreground mt-1">Lancez une partie ou rejoignez d'autres joueurs</p>
            </div>
            <div className="flex gap-2 justify-center">
              <Link to="/jeux"
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-bold active:scale-95 transition-all">
                <Plus className="w-3.5 h-3.5" /> Créer une partie
              </Link>
              <Link to="/jeux"
                className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-secondary text-foreground text-xs font-bold active:scale-95 transition-all border border-border">
                <Gamepad2 className="w-3.5 h-3.5" /> Rejoindre
              </Link>
            </div>
          </div>
          ...
          <ul className="space-y-2">
            {games.slice(0, 6).map((g: any) => {
              const def = GAME_DEFS[g.game_slug || g.slug] ?? { emoji: "🎮", label: g.game_slug || "Jeu" };
              return (
                <li key={g.id}
                  className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl bg-card border border-border p-3">
                  <div className="w-11 h-11 rounded-xl bg-secondary grid place-items-center text-xl shrink-0">
                    {def.emoji}
                  </div>
                  <div className="min-w-0">
                    <div className="font-extrabold text-sm truncate">{def.label}</div>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      <span className="flex items-center gap-0.5 text-[10px] text-muted-foreground font-medium">
                        <Users className="w-3 h-3" /> {g.player_count ?? 0}
                      </span>
                      {g.stake != null && g.stake > 0 && (
                        <span className="flex items-center gap-0.5 text-[10px] font-bold text-amber-500">
                          <Coins className="w-3 h-3" /> {fmtAr(g.stake)}
                        </span>
                      )}
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                        g._state === "live"
                          ? "bg-red-500/10 text-red-500 inline-flex items-center gap-1"
                          : g.is_private ? "bg-orange-500/10 text-orange-500" : "bg-sky-500/10 text-sky-500"
                      }`}>
                        {g._state === "live"
                  ...
                  {(() => {
                    const slug = (g.game_slug || g.slug) as string;
                    ...
                    const handleJoin = async () => {
                      ...
                      if (!isLive) {
                        try {
                          await supabase.rpc(`${slug}_join` as any, { _game_id: g.id } as any);
                        } catch { /* déjà participant ou pleine */ }
                      }
                      navigate({ to: target as any, params: { id: g.id } });
                    };
                    return (
                      <button onClick={handleJoin}
                        className={`shrink-0 flex items-center gap-1 px-3 py-2 rounded-xl font-bold text-xs ${
                          isLive ? "bg-red-500/10 text-red-500" : "bg-primary/10 text-primary"
                        }`}>
                        {isLive ? "Regarder" : "Jouer"} <ChevronRight className="w-3 h-3" />
                      </button>
                    );
                  })()}
                </li>
            ...
            {games.length > 6 && (
              <Link to="/jeux" className="block text-center text-xs font-bold text-primary py-2">
                Voir toutes ({games.length}) →
              </Link>
            )}
          </ul>
        )}
