 ...
 *  Optimistic movement: piece appears at destination instantly on tap (like Ludo). */
...
export function ChessBoard({ fen, myColor, onMove, lastMove, disabled }: ChessBoardProps) {
  // ── Optimistic FEN: we display this instead of the server FEN.
  // When the player moves, we compute the new FEN locally and display it
  // immediately. When the server FEN arrives, we sync (it should match).
  // On error, we roll back to the server FEN.
  const [displayFen, setDisplayFen] = useState(fen);
  const serverFenRef = useRef(fen);

  // Sync display FEN when server FEN changes (bot move, opponent move, reconnection)
  useEffect(() => {
    serverFenRef.current = fen;
    setDisplayFen(fen);
  }, [fen]);

  const chess = useMemo(() => {
    try { return new Chess(displayFen); } catch { return new Chess(fen); }
  }, [displayFen, fen]);
  ...
  useEffect(() => { setSelected(null); setPendingPromotion(null); }, [displayFen]);
  ...
  const commitMove = useCallback(async (from: Square, to: Square, promotion?: string) => {
    ...
    // ── Optimistic: display the new FEN instantly ──
    const optimisticFen = before.fen();
    setDisplayFen(optimisticFen);

    try {
      const uci = from + to + (move.promotion ? move.promotion : "");
      await onMove(uci, move.san, optimisticFen);
    } catch {
      // Roll back to server FEN on error
      setDisplayFen(serverFenRef.current);
    } finally {
