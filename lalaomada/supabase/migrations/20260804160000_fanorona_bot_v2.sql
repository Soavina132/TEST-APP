-- Fanorona Bot v2: improved chain evaluation, chain-potential scoring, depth-2 for high intel
...
DECLARE
  ...
  sim_board jsonb; sim_can_chain boolean; sim_chain_bonus int;
...
BEGIN
  ...
  IF chain_from_v IS NOT NULL THEN
    ...
    FOR ch_dr, ch_dc IN SELECT a,b FROM (VALUES
      ...
      -- Base score: capture count
      ch_score := ch_cap_count * 100;

      -- Determine which capture type to use (more pieces wins, ties go to approach)
      IF jsonb_array_length(ch_lists->'approach') >= jsonb_array_length(ch_lists->'withdrawal') THEN
        ch_best_cap := ch_lists->'approach';
      ELSE
        ch_best_cap := ch_lists->'withdrawal';
      END IF;

      -- For intelligence 4+: check if piece can continue chaining after this capture
      IF v_intelligence >= 4 THEN
        ...
        FOR ch_j IN 0..jsonb_array_length(ch_best_cap) - 1 LOOP
          ch_tmp_board := jsonb_set(ch_tmp_board, ARRAY[((ch_best_cap->ch_j)::int)::text], '0'::jsonb);
        END LOOP;

        -- Check if the piece at ch_nidx can capture again
        ...
        IF ch_can_continue THEN
          ch_score := ch_score + 200;  -- Big bonus for chain continuation
        END IF;

        -- Also penalize if opponent can counter-capture after this chain ends
      ...
      -- Low intelligence: add randomness
      ...
      IF ch_score > ch_best_score THEN
        ...
        ch_best_to := ch_nidx;
        -- Re-evaluate which capture to use for this option
        IF jsonb_array_length(ch_lists->'approach') >= jsonb_array_length(ch_lists->'withdrawal') THEN
          ch_best_cap := ch_lists->'approach';
        ELSE
          ch_best_cap := ch_lists->'withdrawal';
    ...
    ELSE
      -- No capture available in chain: must pass
  ...
  FOR i IN 0..(v_cols * v_rows - 1) LOOP
    ...
    FOR dr, dc IN SELECT a,b FROM (VALUES
      ...
      -- Skip non-capturing moves if capture is mandatory and available
      IF has_capture AND COALESCE(g.mandatory_capture, true) AND cap_count = 0 THEN
        CONTINUE;
      END IF;

      -- Base score: captures * 100 + center bias (prefer central pieces)
      ...
      IF cap_count > 0 THEN
        IF jsonb_array_length(lists->'approach') >= jsonb_array_length(lists->'withdrawal') THEN
          best_cap := lists->'approach';
        ELSE
          best_cap := lists->'withdrawal';
        END IF;
      ELSE
        best_cap := '[]'::jsonb;
      END IF;
      ...
      IF cap_count > 0 THEN
        FOR j IN 0..jsonb_array_length(best_cap) - 1 LOOP
          sim_board := jsonb_set(sim_board, ARRAY[((best_cap->j)::int)::text], '0'::jsonb);
        END LOOP;
      END IF;

      IF v_intelligence <= 2 THEN
        -- Low intelligence: heavy randomness, no lookahead
        score := score + (random() * 50)::int;

      ELSIF v_intelligence = 3 THEN
        -- Medium intelligence: light randomness
        score := score + (random() * 20)::int;

      ELSIF v_intelligence = 4 THEN
        -- High intelligence: chain potential + counter-capture check
        IF cap_count > 0 THEN
          ...
          IF sim_can_chain THEN
            score := score + 150;  -- Chain bonus
          END IF;
        END IF;
        -- Penalize moves that let opponent capture
        IF public._fanorona_player_can_capture(sim_board, opp_color, v_cols, v_rows) THEN
          score := score - 50;
        END IF;

      ELSE
        -- Intelligence 5 (Master): chain potential + opponent best response
        IF cap_count > 0 THEN
          sim_can_chain := public._fanorona_piece_can_capture(
            sim_board, bot_color, nidx, '[]'::jsonb,
            public._fanorona_axis(dr, dc), v_cols, v_rows
          ...
          IF sim_can_chain THEN
            score := score + 200;  -- Bigger chain bonus
          END IF;
        END IF;

        -- Evaluate opponent's best counter-capture
        opp_best_cap := 0;
        FOR opp_i IN 0..(v_cols * v_rows - 1) LOOP
          ...
          END LOOP;
        END LOOP;
        score := score - opp_best_cap * 80;  -- Penalize based on opponent's best capture
      END IF;
  ...
  -- Execute the best move (or pass if none found)
