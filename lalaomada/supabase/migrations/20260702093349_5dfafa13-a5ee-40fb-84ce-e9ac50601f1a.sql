...
CREATE TABLE IF NOT EXISTS public.referral_settings (
  ...
  deposit_bonus_pct numeric NOT NULL DEFAULT 10,
  deposit_min_ar numeric NOT NULL DEFAULT 500,
  win_commission_pct numeric NOT NULL DEFAULT 0,
...
GRANT SELECT, INSERT, UPDATE ON public.referral_fraud_flags TO authenticated;
...
DROP POLICY IF EXISTS "ref_fraud_admin" ON public.referral_fraud_flags;
CREATE POLICY "ref_fraud_admin" ON public.referral_fraud_flags
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT
    (row_number() OVER (ORDER BY vr.active_referrals DESC, vr.total_earned_ar DESC))::int,
    vr.referrer_id, p.pseudo, p.avatar_url,
    vr.active_referrals, vr.total_earned_ar, vr.tier
  FROM public.v_referral_stats vr
  JOIN public.profiles p ON p.id = vr.referrer_id
  WHERE vr.active_referrals > 0 AND (p.is_banned = false OR p.is_banned IS NULL)
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'created_at', re.created_at, 'referee_pseudo', p.pseudo
  ) ORDER BY re.created_at DESC)
  INTO v_events FROM public.referral_events re
  JOIN public.profiles p ON p.id = re.referee_id WHERE re.referrer_id = v_uid;
...
-- _referral_on_deposit trigger
CREATE OR REPLACE FUNCTION public._referral_on_deposit()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_parent uuid; v_verified boolean; v_unlocked boolean;
  v_pct numeric; v_reward numeric; v_mult numeric := 1.0; v_tier text;
  v_daily_cnt int; v_vel_cnt int;
...
BEGIN
  ...
  IF NEW.user_id = v_parent THEN
    ...
    RETURN NEW;
  END IF;
  SELECT COUNT(*) INTO v_vel_cnt FROM public.referral_events
  ...
  IF v_vel_cnt >= v_cfg.auto_flag_velocity THEN
    ...
    RETURN NEW;
  END IF;
  SELECT COUNT(*) INTO v_daily_cnt FROM public.referral_events
    WHERE referrer_id=v_parent AND event_type='first_deposit' AND created_at >= current_date;
  ...
  BEGIN
    ...
    VALUES(v_parent, 'referral', v_reward, NEW.id,
      format('Parrainage %s — %s%% × %s Ar (tier %s)',
        (SELECT pseudo FROM public.profiles WHERE id=NEW.user_id), v_pct, NEW.amount, v_tier));
  ...
  VALUES(v_parent, NEW.user_id, 'first_deposit', v_reward,
    format('%s%% × %s Ar (tier %s × %s)', v_pct, NEW.amount, v_tier, v_mult))
...
CREATE TRIGGER trg_referral_signup AFTER INSERT ON public.profiles
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT f.id, f.referrer_id, f.referee_id, p1.pseudo, p2.pseudo,
...
DECLARE
  ...
  v_reward numeric; v_pct numeric;
BEGIN
...
CREATE OR REPLACE FUNCTION public.admin_update_referral_settings(
  _deposit_bonus_pct numeric DEFAULT NULL, _deposit_min_ar numeric DEFAULT NULL,
  _win_commission_pct numeric DEFAULT NULL,
  _tier_silver_min int DEFAULT NULL, _tier_gold_min int DEFAULT NULL, _tier_diamond_min int DEFAULT NULL,
  _tier_silver_mult numeric DEFAULT NULL, _tier_gold_mult numeric DEFAULT NULL, _tier_diamond_mult numeric DEFAULT NULL,
  _require_phone boolean DEFAULT NULL,
  _max_daily int DEFAULT NULL, _auto_flag_velocity int DEFAULT NULL,
  _enabled boolean DEFAULT NULL, _campaign_label text DEFAULT NULL,
  _campaign_expires timestamptz DEFAULT NULL, _campaign_bonus_pct numeric DEFAULT NULL
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT public.is_admin() THEN RAISE EXCEPTION 'Accès refusé'; END IF;
