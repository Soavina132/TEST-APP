-- ============================================================================
-- Fix : Afficher le dé + cadre du joueur pendant 1 seconde sur no_move
-- Le backend passe le tour immédiatement mais ajoute no_move_display au state.
-- Le frontend utilise cette info pour afficher le dé et le cadre du joueur
-- qui vient de lancer pendant 1 seconde avant de passer visuellement au suivant.
-- ============================================================================

-- ── ludo_roll : passer le tour MAIS garder l'info d'affichage ──
...
BEGIN
  ...
  IF NOT has_move THEN
    -- Passer le tour immédiatement mais garder l'info d'affichage pendant 1 seconde.
    v_display := jsonb_build_object(
      'slot', v_slot,
      'dice', v_dice,
      'until', to_char((now() + interval '1 second') AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"')
    );
    UPDATE public.ludo_participants SET consecutive_sixes=0 WHERE game_id=_game_id AND slot=v_slot;
    st := jsonb_set(st,'{must_move}','false'::jsonb);
    st := jsonb_set(st,'{turn_slot}', to_jsonb(public._ludo_next_slot(_game_id, v_slot, g.max_players)));
...
-- ── ludo_pass : restaurer la version originale (sans no_move special case) ──
