-- Fix: un même compte pouvait apparaître plusieurs fois dans le classement.
-- Cause: leaderboard_winners groupait par (user_id, nom affiché). Si un joueur
-- avait un display_name différent d'une partie à l'autre, le même compte était
-- compté comme plusieurs joueurs distincts.
-- Correction: on groupe uniquement par user_id (1 compte = 1 ligne), et on
-- choisit un nom stable : le pseudo du profil en priorité, sinon le nom de
-- partie le plus récent.
