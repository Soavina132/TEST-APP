-- ═════════════════════════════════════════════════════════════════
-- FIX: ludo_quit — "cannot get array length of a non-array"
-- Date: 2026-08-21 19:32
--
-- Problème: jsonb_array_length(v_pawns) plante si g.state->'pawns'
-- est NULL ou n'est pas un tableau (peut arriver si le state est
-- corrompu ou en cours de transition).
-- ═════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.ludo_quit(_game_id text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ✅ Safety check: only iterate if pawns is a valid JSON array
  IF v_pawns IS NOT NULL AND jsonb_typeof(v_pawns) = 'array' THEN
    ...
    UPDATE public.ludo_games
      ...
      WHERE id = _game_id;
  ELSE
    -- pawns is null or not an array — mark all pawns for this slot as finished
    -- by updating the state directly without iterating
    RAISE NOTICE 'ludo_quit: pawns is not an array (%)', jsonb_typeof(v_pawns);
  END IF;

  UPDATE public.ludo_participants SET finished = true
    WHERE game_id = _game_id AND user_id = v_uid AND is_bot = false;
...
REVOKE ALL ON FUNCTION public.ludo_quit(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ludo_quit(text) TO authenticated;
