-- Fix: _fanorona_play_by_slot had a leftover "IF move_count = 0" branch that
-- forced the turn to pass immediately after a capture on the VERY FIRST move
-- of the game, even when the same piece could legally continue a chain
-- capture. Chain continuation must be evaluated the same way regardless of
-- move_count. This also removes one contributing cause of the bot appearing
-- "stuck" (the main cause was a frontend re-trigger bug, fixed separately).
...
BEGIN
  ...
  ELSE
    ...
    -- regardless of whether this is the very first move of the game.
