-- Fix: ludo_check_timeout must increment T1/T2 and call _ludo_check_afk
-- The DB version was using missed_turns instead of afk_t1/afk_t2

-- ── 1. Fix ludo_check_timeout to increment T1 (no roll) / T2 (no move) ──
...
BEGIN
  ...
  -- Handle power_pending (power-up waiting for choice)
  ...
  -- Also try auto_move if enabled before counting T2
  IF COALESCE((st->>'must_move')::boolean, false) AND COALESCE(g.auto_move, false) THEN
    -- Try to auto-move for the player before counting T2
...
-- ── 2. Fix _ludo_check_afk with full logic (forfeit, pawn-to-yard, warning, solo) ──
