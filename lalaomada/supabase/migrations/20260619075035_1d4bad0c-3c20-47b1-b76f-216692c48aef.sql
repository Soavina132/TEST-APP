...
BEGIN
  ...
  SELECT max_players INTO v_max FROM public.domino_games WHERE id = _game_id AND status = 'open';
  IF v_max IS NULL THEN RETURN; END IF;

  SELECT COUNT(*), COUNT(*) FILTER (WHERE ready)
    INTO v_count, v_ready_count
  FROM public.domino_participants WHERE game_id = _game_id;

  IF v_count = v_max AND v_ready_count = v_max THEN
    PERFORM public.domino_start_game(_game_id);
  END IF;
END;
$$;
