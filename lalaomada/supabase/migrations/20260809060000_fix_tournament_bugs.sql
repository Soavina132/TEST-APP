-- ═══════════════════════════════════════════════════════════════════════
-- FIX: Tournament finals — 4p Ludo KO, 2p pools/Domino, placements, rounds
--
-- Rules:
-- 1. Ludo knockout (ppm>=3): final = 4 players (2 finalists + 2 R1 runner-ups)
-- 2. Pools mode (any game): knockout phase uses 2p matches, final = 2 players
-- 3. Domino (ppm=2): final = 2 players + 3rd place when winners_count >= 3
-- 4. Placements stored for pool + 4p final matches (Ludo: finish_rank, sim: sequential)
-- ═══════════════════════════════════════════════════════════════════════

ALTER TABLE public.tournament_matches
  ADD COLUMN IF NOT EXISTS placements jsonb DEFAULT NULL;

-- _t_build_round: pools mode → 2p knockout matches; knockout → ppm
...
DECLARE
  ...
  v_bye uuid; v_target uuid; v_effective_ppm int;
BEGIN
  ...
  IF n = 1 THEN
    ...
    PERFORM public._t_finish(_tid); RETURN;
  END IF;
  v_effective_ppm := CASE WHEN t.format = 'pools' THEN 2 ELSE t.players_per_match END;
  v_bye := NULL;
  WHILE i <= n LOOP
    v_rest := n - i + 1;
    v_take := LEAST(v_effective_ppm, v_rest);
    IF v_rest = 3 AND t.game_slug = 'ludo' AND v_effective_ppm >= 3 THEN v_take := 3; END IF;
    IF v_rest = 1 AND v_take = 1 THEN v_bye := _ids[i]; i := i + 1; CONTINUE; END IF;
    IF v_rest - v_take = 1 AND v_effective_ppm >= 3 THEN v_take := v_take + 1; END IF;
    v_mno := v_mno + 1;
  ...
  IF v_bye IS NOT NULL THEN
    IF v_effective_ppm >= 3 THEN
      SELECT id INTO v_target FROM public.tournament_matches
       WHERE tournament_id = _tid AND round = _round AND phase = 'final'
         AND array_length(entrant_ids, 1) = 2 ORDER BY random() LIMIT 1;
      IF v_target IS NOT NULL THEN
        UPDATE public.tournament_matches SET entrant_ids = entrant_ids || ARRAY[v_bye] WHERE id = v_target;
      ELSE
        SELECT id INTO v_target FROM public.tournament_matches
         WHERE tournament_id = _tid AND round = _round AND phase = 'final'
           AND array_length(entrant_ids, 1) < v_effective_ppm
...
-- _t_match_finish: store placements for pool + 4p final
...
AS $$
DECLARE
  m public.tournament_matches%ROWTYPE; e uuid;
  v_placements jsonb := '{}'::jsonb; v_game_slug text; v_slot int; v_rank int; v_ppm int; v_pos int := 1;
BEGIN
  ...
  IF _winner IS NULL AND m.phase = 'pool' THEN
    UPDATE public.tournament_matches SET status = 'finished', winner_entrant_id = NULL, is_draw = true, finished_at = now() WHERE id = _match_id;
  ELSE
    UPDATE public.tournament_matches SET status = 'finished', winner_entrant_id = _winner, is_draw = false, finished_at = now() WHERE id = _match_id;
  END IF;
  ...
  ELSE
    FOREACH e IN ARRAY m.entrant_ids LOOP
      IF m.phase = 'third_place' OR _winner IS NULL OR e <> _winner THEN
        UPDATE public.tournament_entrants SET status = 'eliminated', eliminated_round = m.round WHERE id = e AND status = 'active';
      END IF;
    END LOOP;
  END IF;
  FOREACH e IN ARRAY m.entrant_ids LOOP
...
-- _t_next_round: Ludo KO 4p final; pools/Domino 2p final
...
AS $$
DECLARE
  t public.tournaments%ROWTYPE; ids uuid[]; losers uuid[];
  v_final_players uuid[]; v_r1_match uuid; v_r1_placements jsonb;
  v_entrant uuid; v_rank int; v_runner_ups uuid[];
BEGIN
  ...
  IF array_length(ids,1) = 2 AND NOT EXISTS (SELECT 1 FROM public.tournament_matches WHERE tournament_id = _tid AND phase = 'third_place') THEN
    IF t.players_per_match >= 3 AND t.format = 'knockout' THEN
...
-- _t_finish: use placements for 4p Ludo final ranking
...
DECLARE
  ...
  v_final_match_id uuid; v_final_placements jsonb; v_entrant uuid; v_rank int;
BEGIN
  SELECT * INTO t FROM public.tournaments WHERE id = _tid FOR UPDATE;
  ...
  IF v_tp_win IS NOT NULL THEN
    ...
    SELECT x.eid INTO v_final_loser FROM (
      SELECT unnest(m.entrant_ids) eid, m.winner_entrant_id w, m.round FROM public.tournament_matches m
       WHERE m.tournament_id = _tid AND m.phase = 'final' AND m.status = 'finished'
         AND t.champion_entrant_id = ANY(m.entrant_ids) ORDER BY m.round DESC LIMIT 4) x
     WHERE x.eid IS DISTINCT FROM t.champion_entrant_id LIMIT 1;
  ELSE
    SELECT id, placements INTO v_final_match_id, v_final_placements FROM public.tournament_matches
     WHERE tournament_id = _tid AND phase = 'final' AND status = 'finished' AND array_length(entrant_ids, 1) > 2
  ...
  END IF;
  WITH keyed AS (
    SELECT e.id, CASE WHEN e.id = t.champion_entrant_id THEN 0 WHEN e.id = v_final_loser THEN 1
      WHEN e.id = v_tp_win THEN 2 WHEN e.id = v_tp_lose THEN 3 ELSE 10 END AS k,
      e.eliminated_round, e.created_at FROM public.tournament_entrants e WHERE e.tournament_id = _tid
  ), ranked AS (
    SELECT id, row_number() OVER (ORDER BY k, eliminated_round DESC NULLS FIRST, created_at) AS rk FROM keyed
  )
  UPDATE public.tournament_entrants e SET final_rank = ranked.rk FROM ranked WHERE e.id = ranked.id;
  IF t.entry_fee_ar > 0 THEN v_net := round(t.prize_pool_ar * (100 - t.platform_pct) / 100) + t.admin_prize_pool_ar;
  ELSE v_net := t.admin_prize_pool_ar; END IF;
  ...
  FOR r IN SELECT * FROM public.tournament_entrants WHERE tournament_id = _tid AND final_rank IS NOT NULL AND final_rank <= t.winners_count ORDER BY final_rank LOOP
    ...
    IF v_amt > 0 AND r.user_id IS NOT NULL AND NOT r.is_bot AND NOT t.is_simulation THEN
      PERFORM public.credit_user_balance(r.user_id, v_amt, 'tournament_prize', _tid, 'Récompense tournoi: ' || t.name, jsonb_build_object('rank', i));
    END IF;
    PERFORM public._t_notify(r.id, '🏆 Tournoi terminé', 'Vous terminez ' || i || 'e. Gain : ' || v_amt || ' Ar', '/tournaments/' || _tid);
  END LOOP;
  UPDATE public.tournaments SET status = 'finished', stage = 'done', finished_at = now(), rewards_paid_at = now(),
    platform_cut_ar = CASE WHEN entry_fee_ar > 0 THEN round(prize_pool_ar * platform_pct / 100) ELSE 0 END WHERE id = _tid;
END;
$$;
