-- Replace 1.5s think delay with:
-- - 20-75s random delay after human plays (max 75s, under 80s turn timer)
-- - 2-5s random delay for bot-to-bot chains

-- ================================================================
-- _domino_play_as: 20-75s delay after human move
-- ================================================================
...
BEGIN
  ...
  IF action = 'pass' THEN
    ...
    -- Set thinking delay if next player is a bot (20-75s random)
    ...
    IF next_is_bot THEN
      think_secs := 20 + floor(random() * 56)::int;  -- 20-75 seconds
...
-- ================================================================
-- _domino_bot_step: 2-5s delay for bot-to-bot chains
-- ================================================================
...
BEGIN
  ...
  IF found THEN
    ...
    -- Set thinking delay if next player is also a bot (2-5s for bot-to-bot)
    SELECT COALESCE(dp.is_bot, false) INTO next_is_bot
      FROM public.domino_participants dp WHERE dp.game_id = _game_id AND dp.slot = next_turn;
    IF next_is_bot THEN
      think_secs := 2 + floor(random() * 4)::int;  -- 2-5 seconds
      st := jsonb_set(st, '{bot_think_until}', to_jsonb((now() + make_interval(secs => think_secs))::text), true);
    END IF;
  ...
  -- Set thinking delay if next player is a bot (2-5s for bot-to-bot)
  SELECT COALESCE(dp.is_bot, false) INTO next_is_bot
    FROM public.domino_participants dp WHERE dp.game_id = _game_id AND dp.slot = next_turn;
  IF next_is_bot THEN
    think_secs := 2 + floor(random() * 4)::int;  -- 2-5 seconds
