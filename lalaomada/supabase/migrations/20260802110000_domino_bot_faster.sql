-- Reduce domino bot thinking delays for snappier gameplay
-- _domino_bot_step: initial think 1.5-3.5s → 0.6-1.5s, after draw 0.8-2.0s → 0.4-1.0s
-- _domino_arm_bot_think: 2s → 1s

-- 1) Faster initial think + faster re-think after draw
...
BEGIN
  ...
  IF v_think_until IS NULL OR v_locked_slot IS DISTINCT FROM v_slot THEN
    -- Faster: 0.6-1.5s instead of 1.5-3.5s
    v_delay_ms := 600 + (floor(random() * 900))::int;
  ...
  IF draw_mode = 'with' AND jsonb_array_length(stock) > 0 THEN
    ...
    -- Faster re-think after draw: 0.4-1.0s instead of 0.8-2.0s
...
-- 2) Faster arm bot think: 1s instead of 2s
...
BEGIN
  ...
  v_state := jsonb_set(v_state, '{bot_think_until}', to_jsonb((now() + interval '1 second')::text), true);
  RETURN v_state;
END;
$function$;
