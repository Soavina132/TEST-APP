-- Fix: canonicalize bot placements exactly like human placements.
--
-- Older bot moves stored raw tiles plus a `flipped` hint. When a bot prepended a
-- tile on the left and the matching pip was the first value (ex: left end 3,
-- tile [3,5]), the board array could start with [3,5] even though the visual
-- chain needed [5,3]. Client-side reconstruction then showed wrong adjacent
-- pips. Store every played bot tile already oriented in visual chain order:
--   left prepend  => [new_outer, old_left]
--   right append  => [old_right, new_outer]
