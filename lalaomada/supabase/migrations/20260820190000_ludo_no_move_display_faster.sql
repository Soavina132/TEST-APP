-- ═════════════════════════════════════════════════════════════════
-- Réduction du délai no_move_display : 2s → 1s (serveur)
--
-- Le frontend affiche "PAS DE COUP" pendant 500ms puis auto-passe
-- à 400ms. Le fallback serveur (ludo_check_timeout) passe après
-- le délai "until". On le réduit à 1s pour matcher.
-- ═════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF jsonb_array_length(v_movable) = 0 THEN
    IF v_isbot THEN
      ...
      RETURN st;
    ELSE
      -- Set no_move_display for the frontend to show "PAS DE COUP" and auto-pass
      -- Reduced from 2s to 1s to match faster frontend (500ms visual, 400ms auto-pass)
      st := jsonb_set(st, '{no_move_display}', jsonb_build_object(
        'slot', v_slot,
        'dice', v_dice,
        'until', to_char(now() AT TIME ZONE 'UTC' + interval '1 second',
