-- ═══ FIX: _domino_bot_step ne met pas à jour first_tile_idx quand le bot joue à gauche ═══
-- Quand le bot préfixe une tuile au board (side=left), first_tile_idx doit être
-- incrémenté de 1, comme le fait domino_play (humain). Sans ça, first_tile_idx
-- pointe sur la mauvaise tuile → le serpent se reconstruit autour du mauvais
-- centre → saut visuel brutal.
--
-- Cette migration recrée _domino_bot_step avec le tracking de first_tile_idx.
