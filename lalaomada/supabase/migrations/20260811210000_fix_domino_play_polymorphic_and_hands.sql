...
-- Bug 1: NULLIF(st->'left_end','') utilisait -> (jsonb) au lieu de ->> (text)
--   NULLIF ne peut pas comparer jsonb avec text → erreur polymorphique
-- Bug 2: st := jsonb_set(st->'hands', ...) remplaçait TOUT l'état par
--   seulement les mains, perdant board/turn_slot/etc.
--   Correct: st := jsonb_set(st, '{hands}', jsonb_set(st->'hands', ...))
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_action = 'play' THEN
    ...
    st := jsonb_set(st, '{hands}', jsonb_set(st->'hands', ARRAY[v_slot::text], hand));
