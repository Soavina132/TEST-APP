-- ─────────────────────────────────────────────────────────────────────────────
-- Fix: Domino match nul dans un jeu à points → aucun point attribué,
-- la partie continue avec une nouvelle manche.
--
-- Comportement précédent : quand _winner_slot IS NULL (pips égaux),
-- _domino_end_round appelait toujours _domino_finalize(NULL) et terminait
-- la partie, même en mode target_score.
--
-- Nouveau comportement :
--   • Jeu sans points (target_score = 0) : comportement inchangé →
--     match nul, pot remboursé, partie terminée.
--   • Jeu à points (target_score > 0) : aucun point attribué pour cette
--     manche, phase "reveal" lancée, nouvelle manche démarrée ensuite
--     comme après un round normal.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF _winner_slot IS NULL THEN
    ...
    -- Jeu à points : aucun point attribué, nouvelle manche
    IF COALESCE(g.target_score, 0) > 0 THEN
      ...
      st := jsonb_set(st, '{last_round}', jsonb_build_object(
        ...
        'blocked',     true,
        'draw',        true,
        'final',       false          -- la partie continue
      ));
    ...
    -- Jeu sans points : fin de partie, pot remboursé
    st := jsonb_set(st, '{last_round}', jsonb_build_object(
      'winner_uid',  NULL::text,
      'round_score', 0,
      'hand_pips',   hand_pips,
  ...
  -- Jeu sans points : fin directe
  IF COALESCE(g.target_score, 0) <= 0 THEN
    PERFORM public._domino_finalize(_game_id, _winner_slot);
    RETURN;
  END IF;

  -- Jeu à points : cumul du score
