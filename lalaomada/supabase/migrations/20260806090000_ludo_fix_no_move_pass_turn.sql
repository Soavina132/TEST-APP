-- ============================================================================
-- Fix : Passage de tour immédiat quand le dé ne permet aucun mouvement
-- + Fix : Suppression du texte "Tu as fait" côté frontend
-- ============================================================================

-- ── ludo_roll : passer le tour immédiatement pour les humains aussi ──
...
BEGIN
  ...
  IF NOT has_move THEN
    -- Humain ou bot : passer le tour immédiatement.
    -- On garde le dé visible (ne pas mettre à null) pour que le joueur
    -- voie ce qu'il a tiré. Le dé sera effacé au prochain lancer.
    st := jsonb_set(st,'{must_move}','false'::jsonb);
    st := jsonb_set(st,'{turn_slot}', to_jsonb(public._ludo_next_slot(_game_id, v_slot, g.max_players)));
    st := jsonb_set(st,'{turn_started_at}', to_jsonb(to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"')));
