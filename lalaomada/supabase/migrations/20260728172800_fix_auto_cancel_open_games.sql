-- Fix: _auto_cancel_open_games crashait (NULL user_id pour les bots), et
-- oubliait poker + petanque. On filtre user_id IS NOT NULL AND is_bot=false
-- avant d'écrire dans transactions/profiles, et on ajoute les 2 jeux manquants.
...
-- Rattrapage immédiat : nettoyer toutes les parties déjà expirées
