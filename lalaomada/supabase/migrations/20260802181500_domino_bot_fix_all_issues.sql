-- Fix 1: _domino_bot_step must check bot_think_before playing
-- Fix 2: _domino_autoplay_bots must filter out games where bot is still thinking
-- Fix 3: domino_play_and_bot must not bypass think delay

-- ================================================================
-- Fix 1: Add bot_think_until check to _domino_bot_step
-- ================================================================
...
BEGIN
  ...
  -- Check bot thinking delay — if still thinking, wait
...
-- ================================================================
-- Fix 2: _domino_autoplay_bots filters out games where bot is thinking
-- ================================================================
  ...
  WHILE attempts < 10 LOOP
    SELECT dg.id INTO g
     ...
     WHERE dg.status = 'playing' AND dp.is_bot = true AND dg.current_turn = dp.slot
       AND dp.forfeited = false
       AND (dg.state->>'bot_think_until' IS NULL 
            OR NULLIF(dg.state->>'bot_think_until', '') IS NULL
            OR (NULLIF(dg.state->>'bot_think_until', ''))::timestamptz <= now())
     LIMIT 1;
...
-- ================================================================
-- Fix 3: domino_play_and_bot — don't bypass think delay
-- ================================================================
...
BEGIN
  ...
  -- Bot will be triggered by frontend via domino_tick after bot_think_until expires
