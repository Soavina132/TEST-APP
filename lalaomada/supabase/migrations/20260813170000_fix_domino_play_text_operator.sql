-- ============================================================
-- FIX: "operator does not exist: text ->> integer" dans domino_play
-- 
-- Cause: domino_play utilisait ->> (retourne text) au lieu de ->
-- (retourne jsonb) pour accéder aux éléments du tableau de main,
-- puis essayait d'utiliser ->> sur le résultat text → erreur.
--
-- domino_bot_execute utilise correctement -> (jsonb) puis ->> (text).
-- ============================================================
...
BEGIN
  ...
  IF _action = 'play' THEN
    ...
    FOR _i IN 0..jsonb_array_length(_hand)-1 LOOP
      -- FIX: utiliser -> (retourne jsonb) au lieu de ->> (retourne text)
