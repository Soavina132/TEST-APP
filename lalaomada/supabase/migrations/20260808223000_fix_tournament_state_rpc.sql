-- ═══════════════════════════════════════════════════════════════════════
-- FIX: tournament_state() was rewritten in 20260807120000_tournament_rules.sql
-- with field names from an old/different schema (e.customer.seed already
-- fixed, but also: w.is_bot, m.winner_id/loser_id/scheduled_at,
-- pe.tournament_id/losses/draws/points_for/points_against/pts, prize_4_pct
-- via non-existent columns) → every call to this RPC threw a Postgres error,
-- so the tournament detail page never loaded (infinite spinner = "rien ne
-- s'affiche" when clicking a tournament card).
--
-- This version uses only columns that actually exist on the current tables,
-- adds computed aliases (winner_id/loser_id, scheduled_at) for frontend
-- compatibility, and groups pools as { pool, players } like the UI expects.
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Matches (with computed winner_id/loser_id/scheduled_at aliases for frontend compat)
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
