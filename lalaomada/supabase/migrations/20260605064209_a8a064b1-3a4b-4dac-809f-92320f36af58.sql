...
-- Fix I: ludo_roll — do NOT increment afk_t2 when no move is possible
...
BEGIN
  ...
  IF NOT has_move THEN
    -- No legal move possible after rolling: NOT counted as AFK T2 (forced skip).
    st := jsonb_set(st,'{must_move}','false'::jsonb);
    st := jsonb_set(st,'{dice}','null'::jsonb);
    st := jsonb_set(st,'{turn_slot}', to_jsonb(public._ludo_next_slot(_game_id, v_slot, g.max_players)));
...
-- Fix II: list_live_games includes private games
CREATE OR REPLACE FUNCTION public.list_live_games()
RETURNS TABLE(id uuid, max_players integer, stake numeric, pot numeric, players_count integer, spectators_count integer, started_at timestamptz, mode text)
