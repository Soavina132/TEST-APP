-- ============================================================
-- FIX CRITICAL: ludo_roll ne mettait pas à jour turn_started_at
-- quand aucun mouvement n'était possible (ex: tous les pions au yard,
-- dé != 6). Cela causait une boucle infinie :
--   1. Joueur lance le dé → pas de mouvement possible → tour passe
--   2. turn_started_at reste ancien → timeout se déclenche immédiatement
--   3. Le joueur suivant est auto-passé → boucle infinie
--
-- FIX: Ajouter turn_started_at dans la branche "no_move" de ludo_roll
-- ============================================================
...
BEGIN
  ...
  IF NOT has_move THEN
    -- No legal move: skip turn, BUT update turn_started_at for the NEXT player!
    -- (This was the bug: turn_started_at was not updated, causing infinite timeout loops)
    st := jsonb_set(st,'{must_move}','false'::jsonb);
    st := jsonb_set(st,'{dice}','null'::jsonb);
    st := jsonb_set(st,'{turn_slot}', to_jsonb(public._ludo_next_slot(_game_id, v_slot, g.max_players)));
    st := jsonb_set(st,'{turn_started_at}', to_jsonb(to_char(now() AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS"Z"')));
