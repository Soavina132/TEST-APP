-- ============================================================
-- Migration: Rami bot — smart AI with meld detection + fast timing
--
-- PROBLEMS:
--   1. Bot never melds → can't win, infinite draw/discard loop
--   2. Bot waits 60s (turn_deadline) before playing → way too slow
--   3. Bot discards highest card → no strategy, wastes useful cards
--
-- FIX:
--   1. rami_bot_play: smart draw (consider discard pile), find & play
--      valid melds (sets + runs with jokers), smart discard (deadwood)
--   2. rami_bot_tick_all: runs every 5s, plays bots after 2s delay
--   3. Cron schedule for rami_bot_tick_all
--
-- CARD ENCODING:
--   Card c: base = c % 56, suit = base / 13 (0-3), rank = base % 13 (0-12)
--   Jokers: base >= 52 (cards 52-55 and 108-111)
--   2 decks: 0-55 (deck 1), 56-111 (deck 2)
-- ============================================================

-- ═══════════════════════════════════════════════════════════
-- rami_bot_play: ONE phase per call (draw OR play+discard)
-- ═══════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF _g.turn_phase = 'draw' THEN
    ...
    _action_log := COALESCE(_state->'action_log','[]'::jsonb) ||
      jsonb_build_object('t','bot_draw','p',_uid::text,'from',CASE WHEN _card IS NOT NULL AND _card = ANY(_pile) THEN 'discard' ELSE 'deck' END,'ts',extract(epoch from now())::bigint);
    ...
    IF _distinct_suits >= 3 THEN
      ...
      IF public._rami_meld_type(_set_cards, _g.joker_mode, _g.random_joker) IS NOT NULL THEN
        _melds := _melds || jsonb_build_array(
          jsonb_build_object('player',_uid::text,'cards',to_jsonb(_set_cards),'type','set'));
    ...
    WHILE _i <= array_length(_suit_cards,1) - 2 LOOP
      ...
      IF _run_len >= 3 THEN
        ...
        IF public._rami_meld_type(_run_cards, _g.joker_mode, _g.random_joker) IS NOT NULL THEN
          _melds := _melds || jsonb_build_array(
            jsonb_build_object('player',_uid::text,'cards',to_jsonb(_run_cards),'type','run'));
  ...
  IF array_length(_melded_cards,1) > 0 THEN
    ...
    _action_log := COALESCE(_state->'action_log','[]'::jsonb) ||
      jsonb_build_object('t','bot_meld','p',_uid::text,'n',array_length(_melded_cards,1),'ts',extract(epoch from now())::bigint);
    _state := jsonb_set(_state, '{action_log}', _action_log);
  ...
  IF array_length(_hand,1) IS NULL OR array_length(_hand,1) = 0 THEN
    ...
    IF _won THEN
      ...
      INSERT INTO public.transactions(user_id,type,amount,ref_id,note)
        VALUES (_uid,'rami_win',_payout,_game_id,'Win rami (bot)');
  ...
  _action_log := COALESCE(_state->'action_log','[]'::jsonb) ||
    jsonb_build_object('t','bot_discard','p',_uid::text,'card',_best_card,'ts',extract(epoch from now())::bigint);
  _state := jsonb_set(_state, '{discards}', _discards);
...
GRANT EXECUTE ON FUNCTION public.rami_bot_play(uuid) TO authenticated;

-- ═══════════════════════════════════════════════════════════
-- rami_bot_tick_all: runs every 5s, plays bots after 2s delay
-- ═══════════════════════════════════════════════════════════
  ...
  LOOP
    BEGIN
      ...
      IF _is_bot AND _updated IS NOT NULL AND now() - _updated >= interval '2 seconds' THEN
        PERFORM public.rami_bot_play(_g_id);
      END IF;
