-- Fix: _domino_finalize crashes when winner is a bot (user_id not in users table)
...
AS $func$
DECLARE
  g record; winner_uid uuid; v_is_bot boolean := false; payout numeric;
  p record; n_active integer; refund_each numeric;
  st jsonb;
BEGIN
  ...
  IF _winner_slot IS NULL THEN
    ...
    IF n_active > 0 AND g.pot > 0 THEN
      ...
      FOR p IN SELECT user_id FROM public.domino_participants 
               WHERE game_id = _game_id AND forfeited = false AND user_id IS NOT NULL AND is_bot = false LOOP
  ...
  -- FIX: Skip transactions for bots (their user_id is not in users table)
