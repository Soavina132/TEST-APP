-- Fix: when `side` is not supplied (auto-select), the left branch could match
-- and set `new_left` but never assigned `side := 'left'`. The final
--   new_left := CASE WHEN side = 'left' THEN new_left ELSE le END
-- then reverted `new_left` back to the previous end, exposing a wrong pip on
-- the board (e.g. a tile [3|5] placed against a 3 kept the 3 exposed while the
-- 5 was on the board — allowing an incompatible next tile to be attached).
...
BEGIN
  ...
  ELSE
    ...
    END IF;
    -- Defensive: side must be assigned by now, but guard just in case.
