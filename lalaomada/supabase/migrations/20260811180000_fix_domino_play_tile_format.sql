-- Fix: domino_play now accepts both tile_idx (int) and tile ([a,b] array) formats
-- The frontend sends { action: "play", tile: [3, 5] } not { action: "play", tile_idx: 2 }
-- Previously tile_idx was NULL → a/b were NULL → "Tuile ne match ni gauche ni droite"
...
BEGIN
  ...
  IF v_action = 'play' THEN
    ...
    --   1) { action:"play", tile_idx: 2 }  (old/legacy)
    --   2) { action:"play", tile: [3, 5] }  (current frontend)
    IF _move ? 'tile_idx' AND (_move->>'tile_idx') IS NOT NULL THEN
      v_tile_idx := (_move->>'tile_idx')::INT;
    ELSIF _move ? 'tile' THEN
      -- Frontend sends tile values [a, b] — find matching index in hand
      tile := _move->'tile';
      v_tile_idx := -1;
    ...
    b := (tile->>1)::INT;
    board := st->'board';
    -- FIX: ->> (text) au lieu de -> (jsonb) pour NULLIF
    ...
    -- FIX: mettre à jour hands DANS st, ne pas remplacer st par hands
  ...
  ELSIF v_action = 'draw' THEN
    ...
    hand := hand || tile;
    -- FIX: mettre à jour hands DANS st
    st := jsonb_set(st, ARRAY['hands'], jsonb_set(st->'hands', ARRAY[v_slot::text], hand));
    v_playable := public._domino_playable_tiles(st, v_slot);
    st := jsonb_set(st, ARRAY['playable_tiles'], v_playable);
...
GRANT EXECUTE ON public.domino_play TO authenticated;
