-- ═════════════════════════════════════════════════════════════════
-- FIX: ludo_quit — drop duplicate text version, fix uuid version
-- Date: 2026-08-22 00:05
--
-- Problème: Deux versions de ludo_quit (text + uuid) causaient
-- "Could not choose the best candidate function".
-- Solution: Drop la version text, fix la version uuid avec check
-- jsonb_typeof avant jsonb_array_length.
-- ═════════════════════════════════════════════════════════════════

-- Drop the text version (duplicate)
DROP FUNCTION IF EXISTS public.ludo_quit(_game_id text);

-- Recreate the uuid version with array length safety check
