-- Fix: domino_tick didn't arm bot_think_until when advancing turn after
-- human timeout — bot was stuck until the 30s turn_deadline expired
-- Now calls _domino_arm_bot_think in all 5 places where current_turn changes:
--   1. board_empty + required_slot redirect (before bot check)
--   2. cur_uid IS NULL (empty slot)
--   3. skips >= max + board_empty + required_slot (forfeit path)
--   4. skips < max + board_empty + required_slot (warning path)
--   5. final fallback (advance to next playable slot)
...
BEGIN
  ...
  IF board_empty AND required_slot IS NOT NULL AND required_slot <> g.current_turn THEN
    ...
    UPDATE public.domino_games
       SET state = g.state, current_turn = required_slot,
  ...
  IF cur_uid IS NULL THEN
    ...
    IF _next IS NOT NULL THEN
      ...
      UPDATE public.domino_games SET state = g.state, current_turn = _next,
        turn_deadline = now() + (_cfg.turn_timer_seconds || ' seconds')::interval WHERE id = _game_id;
    END IF;
    RETURN;
  END IF;

  -- ═══ Joueur humain SANS tuile jouable → pioche automatique ═══
