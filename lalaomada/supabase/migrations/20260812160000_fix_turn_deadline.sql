-- Migration: 20260812160000_fix_turn_deadline
-- Fix: domino_play ne mettait pas turn_deadline → le timer 30s ne fonctionnait pas
-- Ajout de turn_deadline dans les UPDATE de domino_play (play + pass)
-- Ajout de turn_deadline dans _domino_visible pour que le frontend puisse l'utiliser
...
BEGIN
  ...
  IF v_action = 'play' THEN
    ...
    UPDATE public.domino_games
      SET state=st, current_turn=v_next_slot,
          turn_deadline = now() + (v_cfg.turn_timer_seconds || ' seconds')::interval
      WHERE id=_game_id;
    RETURN public._domino_visible(_game_id);
  ...
  ELSIF v_action = 'draw' THEN
    ...
    -- DRAW: turn stays with the same player, turn_deadline unchanged
...
-- Update _domino_visible to include turn_deadline in the response
...
BEGIN
  ...
  v_visible := jsonb_build_object(
    ...
    'turn_deadline', to_jsonb(COALESCE(g.turn_deadline::text, ''))
  );
...
REVOKE EXECUTE ON FUNCTION public._domino_visible(uuid) FROM anon;
