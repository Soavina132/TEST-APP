-- Speed up domino bot response: reduce think delay and cron fallback interval

-- 1. Reduce cron fallback from 30s to 5s
SELECT cron.alter_job(
  job_id => (SELECT jobid FROM cron.job WHERE jobname = 'domino_tick_all'),
  schedule => '*/5 * * * * *'
);

-- 2. Reduce bot think delay from 800-2000ms to 400-1000ms in _domino_bot_step
...
-- 3. Reduce think delay in _domino_arm_bot_think
