-- ─────────────────────────────────────────────────────────────────────────────
-- Fix critique : domino_tick n'appelait JAMAIS _domino_bot_step
--
-- Problème 1 : quand cur_uid IS NULL (bot), domino_tick passait au joueur
--              suivant sans faire jouer le bot.
-- Problème 2 : domino_tick retournait si turn_deadline > now(), or le bot
--              pense pendant 2-3s mais le timer dure 30s. Le frontend
--              déclenche domino_tick après bot_think_until, mais la fonction
--              retournait avant d'arriver au bot.
--
-- Solution : avant la vérification du turn_deadline, on vérifie si le joueur
--             courant est un bot avec un bot_think_until expiré. Si oui, on
--             appelle _domino_bot_step. Et si cur_uid IS NULL (bot), on
--             délègue à _domino_bot_step au lieu de sauter le tour.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── NOUVEAU : vérifier si le joueur courant est un bot ──────────────────
  ...
  IF COALESCE(v_is_bot, false) THEN
    -- Vérifier si le think timer est expiré
    v_think_until := NULLIF(g.state->>'bot_think_until','')::timestamptz;
    IF v_think_until IS NOT NULL AND v_think_until <= now() THEN
      -- Le bot a fini de réfléchir, le faire jouer
      PERFORM public._domino_bot_step(_game_id);
    ELSIF v_think_until IS NULL THEN
      -- Pas de think timer armé — _domino_bot_step va l'armer
      PERFORM public._domino_bot_step(_game_id);
    END IF;
    -- Si le think timer existe mais n'est pas encore expiré, ne rien faire
    RETURN;
  END IF;
